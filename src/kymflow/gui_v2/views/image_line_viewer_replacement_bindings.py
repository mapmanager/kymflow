"""Bindings between ImageLineViewerReplacementView and event bus.

Phase 3: wires the replacement view (ImageRoiWidget + LinePlotWidget) to
...
class ImageLineViewerReplacementBindings:
    """Bind ImageLineViewerReplacementView to event bus.

    Same event subscription pattern as ImageLineViewerBindings but targets
    the replacement view. No SetRoiEditState; VelocityEventUpdate, AddKymEvent,
    DeleteKymEvent use refresh_velocity_events (no Plotly CRUD).
    """

    def __init__(self, bus: EventBus, view: ImageLineViewerReplacementView) -> None:
        self._bus = bus
        self._view = view
