...
from kymflow.core.batch_analysis import preview_batch_table_rows
from kymflow.core.batch_analysis.kym_event_batch import roi_intersection_across_files
from kymflow.core.batch_analysis.types import AnalysisBatchKind, BatchFileResult
...
class KymAnalysisBatchDialog:
    ...
    **Cancel** requests task cancellation. After completion, a **Done** state and **Close**
    allow dismissal.
    ...
    def __init__(
        ...
        *,
        ...
        """Initialize dialog (call :meth:`render` once from the page build).
    ...
    def render(self) -> None:
        """Create the dialog UI in the current NiceGUI context."""
        with ui.dialog() as self._dialog, ui.card().classes("w-full max-w-lg"):
            ...
            with ui.column().classes("w-full gap-1") as self._progress_column:
                ...
                self._progress_bar = ui.linear_progress(value=0.0).props("instant-feedback")
            ...
            self._report_table.visible = False
    ...
    def _row_dict_from_result(self, r: BatchFileResult) -> dict[str, str]:
        """Map a batch result to a table row dict."""
        return {
            "file": (
    ...
    def _apply_preview_table(self) -> None:
        ...
        self._report_table.visible = True
    ...
    def _on_close_click(self) -> None:
        """Close dialog when idle or after batch completed."""
    ...
    def schedule_open(self, kind: AnalysisBatchKind) -> None:
        """Call from a sync UI handler (e.g. toolbar button); captures client then loads/opens async."""
        self._kind = kind
        try:
    ...
    async def _refresh_from_table(self) -> None:
        ...
        Returns:
            Snapshot of displayed kymograph files for this dialog session.
        """
    ...
    async def _run_analyze_async(self, client: Client) -> None:
        ...
        if mode == "existing":
            if self._roi_select is None or self._roi_select.value is None:
                self._notify_client(
                    client,
                    "No common ROI across filtered files — pick another filter or use new ROI mode",
                    type_="warning",
                )
                return
            roi_id = int(self._roi_select.value)
