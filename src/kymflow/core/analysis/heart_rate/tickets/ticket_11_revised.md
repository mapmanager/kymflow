# Ticket 11 (REVISED) — Parallel Batch HR + Persistence + DF Init (Scoped, No Runner Changes)
...
- ✅ Modify: `heart_rate_pipeline.py`  **(HeartRateAnalysis lives here)**
- ✅ Modify (only if needed for serialization helpers already defined here): `heart_rate_analysis.py`
...
- ❌ Any refactors unrelated to the goals below
...
## Part A — HeartRateAnalysis: init-from-df + persistence (in heart_rate_pipeline.py)
...
### A2) Persistence API
Add methods to `HeartRateAnalysis` (in heart_rate_pipeline.py):
...
- If `to_dict()/from_dict()` already exist for:
  - `HRAnalysisConfig`
  - `HeartRateEstimate`
  - `HeartRatePerRoiResults`
  use them; otherwise add them with typed signatures + Google docstrings in the file where those types are defined (likely heart_rate_analysis.py for estimates/config, heart_rate_pipeline.py for results containers).
...
### B3) Core compute functions (single compute core)
Implement as pure functions:

- `compute_hr_for_df(df: pd.DataFrame, *, roi_ids: Optional[Sequence[int]], cfg: HRAnalysisConfig, source_id: str) -> HeartRateFileResult`
...
- Keep process worker functions at module top-level (pickleable).
...
- For minimal="mini": use the existing per-ROI “mini summary” schema produced by `HeartRateAnalysis.get_roi_summary(..., minimal="mini")`.
