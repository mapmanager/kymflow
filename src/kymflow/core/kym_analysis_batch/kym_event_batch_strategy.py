...
class KymEventBatchStrategy:
    ...
    def prepare_file(self, kf: KymImage) -> None:
        ...
        try:
            ...
            logger.warning(
                f"KymEventBatchStrategy.prepare_file: load_channel failed for {getattr(kf, 'path', None)} ch={self._channel}",
                exc_info=True,
            )
    ...
    def process_file(
        ...
        *,
        ...
        try:
            ...
            logger.warning(
                f"Kym-event batch failed (ValueError) for {file_label}: {exc}",
            )
            return BatchFileResult(
                kym_image=kf,
                kind=AnalysisBatchKind.KYM_EVENT,
            ...
            logger.exception(f"Kym-event batch failed for {file_label}")
            return BatchFileResult(
                kym_image=kf,
                kind=AnalysisBatchKind.KYM_EVENT,
                outcome=BatchFileOutcome.FAILED,
