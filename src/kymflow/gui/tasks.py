...
def run_flow_analysis(
    ...
    *,
    ...
    Requires an existing ROI - roi_id must be provided and the ROI must exist on the KymImage.
    ...
    Args:
        ...
        roi_id: Identifier of the ROI to analyze. Must be provided and ROI must exist.
        on_result: Optional callback function called when analysis completes
            successfully. Receives a boolean indicating success.
    """
    # Validate roi_id is provided (not None) before starting worker thread
    if roi_id is None:
...
def run_batch_flow_analysis(
    ...
    *,
    ...
    at any point. Files without ROIs are skipped (not analyzed).

    Args:
        kym_files: Sequence of KymImage instances to analyze. Files without ROIs will be skipped.
    ...
    def _worker() -> None:
        ...
        for index, kf in enumerate(files, start=1):
            ...
            # Get first ROI for this file (skip if no ROIs exist)
            roi_ids = kf.rois.get_roi_ids()
            if not roi_ids:
                # Skip files without ROIs
                per_file_task.message = f"Skipped {kf.path.name}: No ROIs found"
                logger.warning(f"Skipping {kf.path.name}: No ROIs found")
                continue
            
            # Use first ROI for analysis
            roi_id = roi_ids[0]
