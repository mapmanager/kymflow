...
def build_header(context: AppContext) -> None:
    """Build the shared header with navigation and theme toggle.
    
    In sub_pages SPA mode, this header is rendered once and stays visible
    across all page navigation. Buttons are always active since detecting
    the current route dynamically requires additional JavaScript logic.
    
    Args:
        context: Application context with theme management
    """
    def _navigate(path: str) -> None:
        """Navigate to a different page using NiceGUI sub_pages routing."""
        ui.run_javascript(f'window.location.href="{path}"')
    
    def _update_theme_icon() -> None:
        icon = "light_mode" if context.dark_mode.value else "dark_mode"
