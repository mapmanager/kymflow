# sandbox/nicegui-threads/thread_job_runner.py
...
WorkerFn = Callable[[threading.Event, Callable[[int, int, str], None]], TResult]
...
class ThreadJobRunner(Generic[TResult]):
    """
    Minimal background-thread runner for NiceGUI.

    Rules:
    - Worker thread MUST NOT touch NiceGUI UI objects.
    - UI thread polls a queue (via ui.timer) and updates UI safely.
    - Optional "latest job wins" gating via job_id (can be disabled by guarding starts).

    Typical usage in UI:
        runner = ThreadJobRunner[list[KymImage]]()
    ...
    def start(
        ...
        *,
        ...
        If you prefer "never start a second job", guard in the UI using is_running().
        """
        with self._lock:
            self._next_job_id += 1
    ...
    def _ensure_timer(self, ui_timer_factory: Callable[[float, Callable[[], None]], object], poll_interval_s: float) -> None:
    ...
    def _worker_entry(self, job_id: int, cancel_event: threading.Event, worker_fn: WorkerFn[TResult]) -> None:
        try:
            def emit(done: int, total: int, detail: str = "") -> None:
                self._q.put(ProgressMsg(job_id=job_id, done=done, total=total, detail=detail))
    ...
    def _handle_msg(self, msg: WorkerMsg[TResult]) -> None:
        ...
        # "Latest job wins": ignore stale job messages.
        if getattr(msg, "job_id", None) != latest_id:
            return

        if isinstance(msg, ProgressMsg):
            if on_progress:
                on_progress(msg.done, msg.total, msg.detail)
            return
