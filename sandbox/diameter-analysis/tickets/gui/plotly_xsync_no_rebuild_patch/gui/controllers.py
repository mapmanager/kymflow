...
class AppController:
    ...
    def _rebuild_figures(self) -> None:
        ...
        """Compute the full x-range (seconds) for the current image, or None if unavailable."""
        if self.state.img is None:
            return None
    ...
    def _apply_xrange_without_rebuild(self, x_range: tuple[float, float] | None) -> None:
        """Apply an x-range to existing figures (no rebuild, do not touch y-axes)."""
        if self.fig_img is None or self.fig_line is None:
            # Nothing to sync yet (figures are built on demand elsewhere).
            return

        if x_range is None:
            # Let Plotly autorange naturally by clearing explicit range.
            # We intentionally do NOT rebuild figures here.
        ...
        x0, x1 = x_range
        from .plotting import set_xrange

        self.fig_img = set_xrange(self.fig_img, x0, x1)
        self.fig_line = set_xrange(self.fig_line, x0, x1)

    def on_relayout(self, source: str, relayout: dict) -> None:
        """Handle Plotly relayout events for x-axis syncing.

        Important: this must NOT rebuild figures (rebuild resets y-axes via autorange).
        We only update x-axis range on the already-built figure dicts.
        """
        if self.state._syncing_axes:
            return

        # Plotly sends a variety of keys; we only care about x-range changes and autorange resets.
        new_range = None
        autorange_reset = False

        try:
            # Zoom / pan range (explicit)
            if "xaxis.range[0]" in relayout and "xaxis.range[1]" in relayout:
                new_range = (float(relayout["xaxis.range[0]"]), float(relayout["xaxis.range[1]"]))
            elif "xaxis.range" in relayout and isinstance(relayout["xaxis.range"], (list, tuple)) and len(relayout["xaxis.range"]) == 2:
                new_range = (float(relayout["xaxis.range"][0]), float(relayout["xaxis.range"][1]))

            # Double-click reset / autoscale
            if relayout.get("xaxis.autorange") is True:
                autorange_reset = True
        except Exception:
            # Ignore malformed relayout payloads
            return
        ...
        target = None
        if autorange_reset:
            # Keep Plotly-like behavior: reset to full range (seconds) but do not touch y.
